                ...
                >
                    <div className="p-6">
                        <div className="flex items-center justify-between mb-8">
                            <h2 className="text-xl font-black text-white">Send USDC</h2>
                        ...
                        {status === "idle" || status === "loading" ? (
                            <div className="space-y-6">
                                <div>
                                    <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 block">Recipient Address</label>
                                    <input
                                        type="text"
                                        placeholder="0x..."
                                        value={address}
                                        onChange={(e) => setAddress(e.target.value)}
                                        className="w-full bg-[#2c2c2e] border border-white/5 rounded-2xl p-4 text-white text-sm outline-none focus:border-indigo-500 transition-colors font-mono"
                                    />
                                </div>
                                ...
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        ...
                                        <span className="text-[10px] text-indigo-400 font-bold">Balance: {balance} USDC</span>
                                    </div>
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full bg-[#2c2c2e] border border-white/5 rounded-2xl p-4 text-white text-2xl font-black outline-none focus:border-indigo-500 transition-colors"
                                        />
                                        <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                                            <span className="text-xs font-bold text-gray-500">USDC</span>
                                            <button
                                                onClick={() => setAmount(balance)}
                                                className="bg-indigo-500/10 text-indigo-400 text-[10px] font-black px-2 py-1 rounded-md hover:bg-indigo-500/20 transition-colors"
                                            >
                                                MAX
                                            </button>
                                        </div>
                                ...
                                <Button
                                    ...
                                    className="w-full h-14 bg-indigo-600 hover:bg-indigo-500 rounded-2xl text-white font-bold flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/20 active:scale-[0.98] transition-all disabled:opacity-50"
                                >
                                    {isLoading ? (
                                        <>
                            ...
                            <div className="py-8 flex flex-col items-center text-center">
                                ...
                                <p className="text-gray-400 text-sm mb-6">Your tokens are on their way.</p>

                                {txHash && (
                                    <a
                                        href={`https://testnet.arcscan.app/tx/${txHash}`}
                                        target="_blank"
                                        className="text-indigo-400 text-xs font-mono break-all mb-8 block hover:underline"
                                    >
                                        View: {txHash.slice(0, 20)}...
                                    </a>
                                )}

                                <Button onClick={onClose} className="w-full bg-[#2c2c2e] hover:bg-[#3a3a3c] text-white rounded-2xl h-12">
                                    Done
                                </Button>
                            </div>
                        ) : (
                            <div className="py-8 flex flex-col items-center text-center">
                                ...
                                <Button onClick={() => setStatus("idle")} className="w-full bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl h-12">
                                    Try Again
                                </Button>
                            </div>
                        )}
